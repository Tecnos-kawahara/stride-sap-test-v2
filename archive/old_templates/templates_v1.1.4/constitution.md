...
- 承認済み `basic_design.md` および承認済み `process.bpmn` を、後続（spec/plan/tasks）の要件正本として扱う。

# 2. Definitions（最小用語）
- Basic Design: HITL判断のための設計ハブ（要件の構造化・トレーサビリティ・未確定・リスク）
- BPMN: ハイレベル業務フロー（Camunda 8 / Zeebe向け）。HITL承認後に後続生成の正本になる。
- Spec: WHAT/WHY（HOW禁止）
...
* 推奨：自然言語要件 → basic_design.md（HITL承認）→ process.bpmn（HITL承認）→ spec/plan/tasks の順で生成する。
...
* 推奨：ファイル名はスネークケースに統一する（例：`basic_design.md` / `process.bpmn` / `spec.md` / `plan.md` / `tasks.md`）。
