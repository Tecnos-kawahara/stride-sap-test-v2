...
## 0.1 目的・位置づけ（3〜5行）
- 本書は「要件要求 → 基本設計（HITL判定） → SDD（spec/plan/tasks）」のハブである。
- 人間は本書で、設計妥当性・抜け漏れ・リスク・例外（憲法）を判断する。
- AIは本書を参照して spec/plan/tasks を生成・更新する（ただし正本は各ドキュメントの Canonical YAML）。
...
|---|---|---|---|
| basic_design.md | specs/XXX_feature_name/basic_design.md | HITL判断ハブ | PO/TL |
...
# Part B. HOW Policy（Planに渡す“実装方針”のインターフェース）
