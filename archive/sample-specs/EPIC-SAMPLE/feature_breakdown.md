...
feature_breakdown:
  ...
  decomposition_rationale:
    ...
    decisions:
      ...
      - decision_id: "FBD-DEC-002"
        ...
        rationale: "mcframe在庫引当は受注登録フローの一部。独立したビジネス価値を持たないため、FEAT-ERP-OMS内のCT-API-03として管理"
        alternatives_considered:
  ...
  cross_team_tests:
    - test_id: "TS-INT-02"
      ...
      type: "integration"
      participating_features: ["FEAT-ERP-OMS"]
...
1. **Single Feature = Single Team**: FEAT-ERP-OMS は TEAM-SALES が完全オーナーシップ
...
### 2.1 FEAT-ERP-OMS: mcframe受注管理アドオン
...
└─ WI-ERP-SAMPLE-001 (TEAM-SALES, autopilot)
   ├─ 登録ボタン配置変更
   ├─ 確認ダイアログ実装
   ├─ E2Eテスト追加（TS-E2E-01）
...
└─ WI-ERP-SAMPLE-002 (TEAM-SALES, validate)
   ├─ 権限マトリクス実装
   ├─ 承認フロー状態マシン
   ├─ SoD検証ロジック
   ├─ 監査ログミドルウェア
...
- `specs/sample_erp_addon/` - 全Feature仕様

### 7.2 Traceability Summary

```
Epic: EPIC-SAMPLE
└─ Feature: FEAT-ERP-OMS (TEAM-SALES, standard)
