...
`sdd-templates/examples/process_bpmn_advanced_example.bpmn` には、次の advanced 例があります。
...
このサンプルは **default テンプレートではなく、任意導入の参考例** として扱ってください。

---

## 8. EPIC BPMN vs FEAT BPMN の使い分け

Tecnos-STRIDE では、BPMN を 2 種類に分けて使います。

| 種別 | ファイル | 目的 | 検証 |
|------|---------|------|------|
| **FEAT BPMN** | `specs/<feature>/process.bpmn` | 単一 feature の実装フロー | `stride_lint.py`（executable BPMN ルール） |
| **EPIC BPMN** | `epics/<EPIC>/epic_flow.bpmn` | チーム間・システム間の連携 overview | `epic_validator.py`（軽量検証） |
...
# → epics/EPIC-ORDER/epic_flow.bpmn が生成される
```

**検証:**
```bash
python3 sdd-templates/tools/epic_validator.py validate epics/EPIC-ORDER/
# → epic_flow.bpmn の collaboration 構造を軽量検証
```

**サンプル:** `epics/EPIC-SAMPLE/epic_flow.bpmn`

---

> Camunda 8.8 BPMN Practice Guide for SDD Templates
