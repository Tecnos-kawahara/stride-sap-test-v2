...
4. CLAUDE.md（このファイル、プロジェクト）    # プロジェクト運用ルール
5. SDD_MANIFESTO.md                           # ツール非依存 SDD コア
6. CLAUDE_WORKFLOW.md                         # Claude Code 固有補足のみ
7. agent_docs/sdd_guidelines.md               # 詳細ガイドライン
8. agent_docs/commands.md                     # CLI リファレンス
9. ユーザーレベル CLAUDE.md（~/CLAUDE.md 等） # 汎用ルール。SDD Phase Gate と矛盾時は SDD 側優先
...
- 「NEVER save to root folder」 vs CLAUDE.md/SDD_MANIFESTO.md 等のルートファイル
  → このプロジェクトのルート直下ガバナンスファイル (CLAUDE*.md, SDD_*.md, SYMPHONY.md, README.md, pyproject.toml 等) は**既存として保護**、新規はルート禁止を維持。
...
## Extension Pack（MANDATORY — Bootstrap と同時に読むこと）

**作業開始前に `.stride-extensions.yaml` を確認すること。**
拡張パックが有効（`active_extensions` にエントリがある）な場合、
**標準SDDフローだけでは不十分**であり、以下のドキュメントを Bootstrap と同時に読まなければならない。

**読み込み手順（拡張パックが有効な場合）:**
1. `.stride-extensions.yaml` を読み、`active_extensions` を確認する
2. 各拡張パックについて `extensions/<ext>/CLAUDE_<EXT>.md` を読む（拡張固有ルール）
3. 各拡張パックについて `extensions/<ext>/CLAUDE_WORKFLOW_<EXT>.md` を読む（拡張側のPhase別手順）
4. **以降の各Phase作業は、標準フローをベースとしつつ、拡張パックが定義する代替・追加ステップを適用して実行する**

> 拡張パックは標準フローを全面的に置き換えるものではない。
> 拡張パック側に記載があるPhase・ステップについてのみ、代替または追加として適用される。
> 記載がないPhaseは標準フローがそのまま有効。

**拡張パックが有効な状態で、拡張パック側のワークフローを読まずに Phase 作業を開始してはならない。**

## Execution Model (v4.4 + v4.6 Schema-Gated)

**Claude Code = 実行者 (R)** | **人間 = 承認者 (A)**

- Claude Code が全作業を**自律実行**する（init, lint, テスト, Evidence, 自動修正）
...
| **MUST ASK** | 人間の承認後に実行 | DDIC 操作（テーブル/ドメイン/データエレメントの ADT API 経由での登録・変更）— 実行前に対象・内容を提示し人間の承認を得ること。既存 DDIC の破壊的変更（フィールド削除・型変更等）は特に慎重に確認 |
...
| 詳細設定 | `CLAUDE_WORKFLOW.md`（Claude Code 固有）, `SDD_MANIFESTO.md`（ツール非依存ルール） |
...
- Phase Gate hook: See `CLAUDE_WORKFLOW.md` for `.claude/settings.json` setup
