...
workspace:
  ...
  base_branch: "main"             # デフォルトのベースブランチ（Issue で未指定時に使用）
...
| `{{ base_branch }}` | str | マージ先ベースブランチ（Issue body > config > "main"） |
...
| `SYMPHONY_BASE_BRANCH` | マージ先ベースブランチ（未設定時は "main"） |
...
- **ベースブランチ**: Issue body の "Base Branch" フィールド > `workspace.base_branch` 設定 > `"main"` の優先順で決定
...
Enterprise Hierarchy（Epic）を使用する場合、以下の 3 階層ブランチモデルで運用します:

```
main
 └── epic/<EPIC-ID>
      └── feature/<EPIC-ID>/<feature_name>      ← Issue の Base Branch に指定
           ├── symphony/<feature_name>-12  (Phase 1: Design)
           ├── symphony/<feature_name>-13  (Phase 2: Specify)
           ├── symphony/<feature_name>-14  (Phase 3: Tasking)
           └── symphony/<feature_name>-20  (Phase 4: WI-001)
```

Issue テンプレートの "Base Branch" フィールドに Feature ブランチ名を指定することで、
Symphony が作成する worktree ブランチは Feature ブランチから分岐し、PR も Feature ブランチに向けて作成されます。
