...
epic:
  ...
  shared_contracts:
    - contract_ref: "specs/FEAT-ERPSAMPLE/contracts/api.yaml#mcframe-stock-allocation"
  ...
  critical_path:
    - milestone_id: "EM-01"
      dependent_features: ["FEAT-ERPSAMPLE"]
      target_date: "2026-02-10"
      status: "completed"
    - milestone_id: "EM-02"
      dependent_features: ["FEAT-ERPSAMPLE"]
  ...
  epic_flow_descriptions:
    overview:
      purpose: "営業部門とmcframe基幹システム間の受注管理連携フローを俯瞰する"
      scope: "受注データ入力→mcframe在庫引当→承認判定→受注確定の全体フロー"
      out_of_scope: "出荷・請求プロセス、在庫管理マスタメンテナンス"
    participants:
      - participant_id: "Participant_Sales"
        name: "営業部門"
        role: "internal_team"
        responsibility: "受注データの入力、引当結果の確認、承認判定の実施"
        owner: "TEAM-SLS"
      - participant_id: "Participant_ERP"
        name: "mcframe基幹"
        role: "external_system"
        responsibility: "受注データの受信、在庫引当の実行、結果の返却"
        owner: "mcframe運用チーム"
    message_flows:
      - message_flow_id: "MsgFlow_OrderReq"
        summary: "営業部門からmcframeへの受注データ送信"
        payload: "受注ヘッダ(顧客コード, 受注日) + 明細(品目コード, 数量, 単価)"
        contract_ref: "CT-API-01"
        dependency_ref: "DEP-001"
        sla: "P95 < 3s"
        business_expectation: "受注データが欠損なくmcframeに到達し、引当処理が開始されること"
      - message_flow_id: "MsgFlow_OrderRes"
        summary: "mcframeから営業部門への引当結果通知"
        payload: "引当結果(成功/失敗) + 引当数量 + 倉庫ID"
        contract_ref: "CT-API-03"
        dependency_ref: "DEP-001"
        sla: "P95 < 5s"
        business_expectation: "引当結果が正確に返却され、営業担当が即時に確認できること"
  ...
  progress_tracking:
    ...
    artifacts:
      ...
      epic_flow_bpmn: "epics/EPIC-SAMPLE/epic_flow.bpmn"
      dependency_manifest: "epics/EPIC-SAMPLE/DEPENDENCY_MANIFEST.yaml"
      contract_registry: "specs/FEAT-ERPSAMPLE/contracts/"
...
| Activity | TEAM-SLS | Epic Lead | ARCH_BOARD |
|----------|-----------|-----------|------------|
...
|------------|------|------|---------------|----------|--------------|
| FEAT-ERPSAMPLE | mcframe受注管理アドオン | TEAM-SLS | standard | 1 | mcframe API v2 |
...
|---------|------|-----------|
| FEAT-ERPSAMPLE | standard | ERP連携ありだが、既存mcframe APIの上に構築。単一チーム。critical は過剰 |
...
|-------------|------|------|-------|-----------|
| SC-API-MCFRAME | mcframe在庫引当API | API | PLATFORM | TEAM-SLS |
...
- **Status**: completed
- **Features**: FEAT-ERPSAMPLE (WI-ERP-SAMPLE-001)
- **Exit Criteria**:
...
- **Status**: pending
- **Features**: FEAT-ERPSAMPLE (WI-ERP-SAMPLE-002)
- **Exit Criteria**:
  - [ ] 金額別3段階承認フロー実装（AC-US-FEATERPOMS-002-01）
  - [ ] SoD検証ロジック実装・テスト完了
  - [ ] 全操作の監査ログ出力（AC-US-FEATERPOMS-002-02）
...
|----------|------|----------|----------|----------|
| IP-001 | API | mcframe-core | FEAT-ERPSAMPLE | api.yaml#allocateStock |
...
|---------|-------------|-------------|--------|------------|-------|
| ER-001 | mcframe API月次締め期間の応答遅延 | High | Medium | リトライ延長+PENDING_STOCKフォールバック | TEAM-SLS |
| ER-002 | 権限チェック実装の複雑さ→セキュリティリスク | Medium | High | authz_matrix.yaml宣言的定義+validate mode | TEAM-SLS |
| ER-003 | 監査ログ欠落→コンプライアンス違反 | Low | High | ミドルウェア自動出力+TS-INT-04全パターン検証 | TEAM-SLS |
...
|------|---------|-------------|-------|-----|-------------|
| TEAM-SLS | 4 | 89 | 2026-02-01 | 2026-03-15 | 74% |
...
| epic_flow.bpmn | `epics/EPIC-SAMPLE/` | On architecture change | 佐藤三郎 (Epic Lead) |
...
- `specs/FEAT-ERPSAMPLE/` - Feature仕様・契約・テスト

### 10.2 Feature Specs Reference

| Artifact | Path |
|----------|------|
| Basic Design | `specs/FEAT-ERPSAMPLE/basic_design.md` |
| Spec | `specs/FEAT-ERPSAMPLE/spec.md` |
| Plan | `specs/FEAT-ERPSAMPLE/plan.md` |
| Tasks | `specs/FEAT-ERPSAMPLE/tasks.md` |
| BPMN | `specs/FEAT-ERPSAMPLE/process.bpmn` |
| API Contract | `specs/FEAT-ERPSAMPLE/contracts/api.yaml` |
| DB Schema | `specs/FEAT-ERPSAMPLE/contracts/database_schema.yaml` |
| Test Scenarios | `specs/FEAT-ERPSAMPLE/tests/scenarios.yaml` |
| State | `specs/FEAT-ERPSAMPLE/state/state.yaml` |
| Evidence Pack | `specs/FEAT-ERPSAMPLE/implementation-details/evidence_pack.md` |
