...
## Phase 2 前準備: SAP Context Acquisition（任意）

> ⛔ MANDATORY: 実行する場合は `agent_docs/phase2_pre_sap_context.md` を読むこと。
...
0. **Phase 2 前準備（SAP Context Acquisition）の確認**:
   - `specs/<feature>/sap_context.md` が存在するか確認
   - 存在しない場合、Phase 2 前準備のステップ（A1〜C2）を先に実行
