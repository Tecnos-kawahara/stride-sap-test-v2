...
| フィールド | 型 | 有効値 | 説明 | 変更時の影響 |
|---|---|---|---|---|
| `step_id` | 文字列 | フェーズ内のステップ ID（例: `1.5-A1`, `6-3`, `S2-A1`） | ステップの識別子。エビデンスファイル名 `{step_id}__{tool_basename}.evidence.yaml` に使用される | ファイル名が変わるため既存エビデンスとの整合性に注意 |
...
#### phase_1_5: Phase 1.5 -- SAP コンテキスト取得

check_condition: `gate_1_and_2_approved`

| step_id | ツール | 説明 | optional |
|---|---|---|---|
| 1.5-A1 | search.js | SAP オブジェクト検索 | false |
| 1.5-A2 | read.js | SAP ソース参照 | true（参照不要の場合スキップ） |
| 1.5-A3 | pull.js | SAP ソース取得（gated authority: 人間承認必須） | true（新規作成の場合スキップ） |
| 1.5-B2 | sap_context_metadata.py | テーブルメタデータ記録（--tables） | false |
