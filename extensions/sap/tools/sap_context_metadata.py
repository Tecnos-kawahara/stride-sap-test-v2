...
Usage:
  # Phase 1.5 R3: テーブルメタデータ生成（DDL + data_preview）
  python3 extensions/sap/tools/sap_context_metadata.py specs/<feature>/ --tables MCHB MAKT MARA

  # Phase 1.5 1.5-A1 / Phase 4 S1-B1: 選択画面メタデータ生成（ABAP ソースから）
...
def main():
    ...
    if step_id is None:
        print(
            ...
            "Usage: python3 sap_context_metadata.py specs/<feature>/ --tables --step-id 1.5-B2\n"
            "       python3 sap_context_metadata.py specs/<feature>/ --from-spec --step-id 2-B4\n"
            "See: CLAUDE_WORKFLOW_SAP.md Step 1.5-B2 / Step 2-B4",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not args:
        print(
            "Usage:\n"
            "  python3 sap_context_metadata.py specs/<feature>/ --tables [TABLE1 TABLE2 ...] --step-id 1.5-B2\n"
            "  python3 sap_context_metadata.py specs/<feature>/ --from-source --step-id 1.5-B2\n"
    ...
    if table_names:
        ...
        for tname in table_names:
            ...
            # Phase 1.5 R3 で AI が adt_read_table_ddl の結果を記録しているはず
