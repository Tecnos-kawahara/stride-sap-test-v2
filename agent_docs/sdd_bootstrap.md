...
| **MUST ASK** | 人間の承認後に実行 | DDIC 操作（TABL/DTEL/DOMA の ADT API 経由での登録・変更）— 実行前に対象・内容を提示し承認を得ること。既存 DDIC の破壊的変更は特に慎重に確認 |
| **MUST NOT DO** | 絶対禁止 | APPROVAL.md / EPIC_APPROVAL.md / WI-*.approval.md の編集 / Gate スキップ / ERP ランタイムデータへの直接書込（DDIC スキーマ定義を除く） / 画面スクレイピング / 秘密情報コミット / 承認済み成果物の無記録変更 / Phase N+1 ファイルの先行作成 |
...
## 2b. 拡張パック検出（MANDATORY — Phase 作業の前提条件）

**Feature ライフサイクルに入る前に、必ず以下を確認すること。**

### チェック手順

1. `.stride-extensions.yaml` の存在を確認する
2. **存在する場合**: `active_extensions` リストを読む
3. リストにある各拡張パックについて、以下のファイルを**この時点で読み込む**:
   - `extensions/<ext>/CLAUDE_<EXT>.md` — 拡張固有ルール（不可侵）
   - `extensions/<ext>/CLAUDE_WORKFLOW_<EXT>.md` — 拡張側のPhase別手順
4. **以降の各Phase作業は、標準フローをベースとしつつ、拡張パックが定義する代替・追加ステップを適用する**
   - 拡張パック側に記載があるPhase・ステップ → 代替または追加として適用
   - 拡張パック側に記載がないPhase → 標準フロー（セクション3以降）をそのまま実行

> ⛔ **拡張パックが有効な状態で、拡張パック側のワークフローを読まずに Phase 作業を開始してはならない。**

### 拡張パックがない場合

`.stride-extensions.yaml` が存在しないか、`active_extensions` が空の場合は、
以降の標準フロー（セクション3以降）をそのまま実行する。
...
> ⛔ **拡張パック確認**: このPhaseの作業を開始する前に `CLAUDE_WORKFLOW_<EXT>.md` の該当Phaseを確認し、代替・追加ステップがあれば適用すること。
...
- ERP / system-of-record のランタイムデータへの直接書き込み禁止（DDIC スキーマ定義は人間承認付きで許可）
