...
# 2. Linter/Formatter（Oxlint/Oxfmt 推奨、ESLint/Prettier より 50-100倍高速）
npx oxlint --version 2>/dev/null || {
  echo "oxlint not found. Installing..."
  npm install -D oxlint
}
# Oxfmt（フォーマッター）— Prettier の高速代替
npx oxfmt --version 2>/dev/null || npm install -D oxfmt
# ※ ESLint/Prettier も引き続き使用可。高速化が必要な場合に Oxlint/Oxfmt を推奨

# 3. Playwright 確認（E2E用）
