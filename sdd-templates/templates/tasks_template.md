...
> v1.2.5: **完了報告前に spec_refs の全ACと scenarios.yaml を必ず全量確認すること。**
> v4.5: **BDD形式 `acceptance_criteria`（Given-When-Then）で受け入れ条件を構造化する。`escalation_trigger: true` のタスクはHITLレビュー必須。**
...
tasks:
  ...
  # ─────────────────────────────────────────────────────
  # Completion Verification Rules (v1.2.5)
  # ─────────────────────────────────────────────────────
